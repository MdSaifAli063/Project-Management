...
exports.listNotes = async (req, res, next) => {
  try {
    ...
    if (!role) return res.status(404).json({ success: false, message: 'Project not found' });
...
exports.createNote = async (req, res, next) => {
  try {
    const role = await projectRole(req.params.projectId, req.user.userId);
    if (!role) return res.status(404).json({ success: false, message: 'Project not found' });
    if (role !== Roles.ADMIN) return res.status(403).json({ success: false, message: 'Forbidden' });

    const { title, content } = req.body;
    const note = await Note.create({ project: req.params.projectId, title, content, createdBy: req.user.userId });
...
exports.getNote = async (req, res, next) => {
  try {
    ...
    const note = await Note.findOne({ _id: req.params.noteId, project: req.params.projectId }).lean();
...
exports.deleteNote = async (req, res, next) => {
  try {
    ...
    const n = await Note.findOneAndDelete({ _id: req.params.noteId, project: req.params.projectId });
