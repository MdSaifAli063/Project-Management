...
async function isProjectMember(projectId, userId) {
  ...
  const mem = project.members.find(m => m.user.toString() === userId.toString());
...
exports.create = async (req, res, next) => {
  try {
    // Admin only system-level; but we'll allow any logged-in user and add them as admin within their project if they are system admin
    const me = await User.findById(req.user.userId);
    const isSystemAdmin = me.role === Roles.ADMIN;
    if (!isSystemAdmin) return res.status(403).json({ success: false, message: 'Only Admin can create projects' });
...
exports.getOne = async (req, res, next) => {
  try {
    ...
    if (!access) return res.status(404).json({ success: false, message: 'Project not found' });
...
exports.update = async (req, res, next) => {
  try {
    // Admin only
    const me = await User.findById(req.user.userId);
    if (me.role !== Roles.ADMIN) return res.status(403).json({ success: false, message: 'Forbidden' });
...
exports.membersList = async (req, res, next) => {
  try {
    const access = await isProjectMember(req.params.projectId, req.user.userId);
    if (!access) return res.status(404).json({ success: false, message: 'Project not found' });
    const project = await Project.findById(req.params.projectId).populate('members.user', 'name email role').lean();
