...
exports.register = async (req, res, next) => {
  try {
    ...
    await Token.create({ user: user._id, type: 'verify_email', tokenHash, expiresAt });
...
exports.login = async (req, res, next) => {
  try {
    ...
    await Token.create({ user: user._id, type: 'refresh_whitelist', tokenHash, expiresAt: new Date(expMs) });
...
exports.currentUser = async (req, res) => {
  res.json({ success: true, user: { id: req.user.userId, role: req.user.role } });
};
...
exports.refreshToken = async (req, res, next) => {
  try {
    ...
    const tokenHash = hashToken(refreshToken);
    const found = await Token.findOne({ type: 'refresh_whitelist', tokenHash, user: payload.userId });
    if (!found) return res.status(401).json({ success: false, message: 'Refresh token invalidated' });
...
exports.verifyEmail = async (req, res, next) => {
  try {
    ...
    if (!record) return res.status(400).send('Invalid or expired verification token');
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      await Token.create({ user: user._id, type: 'reset_password', tokenHash, expiresAt });
